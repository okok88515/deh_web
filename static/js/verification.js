                ...
                if (content == "poi") {
                    ...
                    if (counts != 0) {
                        ...
                        for (var i = 0; i < counts; i++) {
                            ...
                            style="margin-bottom:0px;"><th><a style="font-size:15px;" href="/poi_detail/' + data.all_poi[i].poi_id + '">' + data.all_poi[i].poi_title + '</a>\
                    ...
                    if (counts != 0) {
                        ...
                        for (var i = 0; i < counts; i++) {
                            ...
                            style="margin-bottom:0px;"><th><a style="font-size:15px;" href="/loi_detail/' + data.all_loi[i].route_id + '">' + data.all_loi[i].route_title + '</a>\
                    ...
                    if (counts != 0) {
                        ...
                        for (var i = 0; i < counts; i++) {
                            ...
                            style="margin-bottom:0px;"><th><a style="font-size:15px;" href="/aoi_detail/' + data.all_aoi[i].aoi_id + '">' + data.all_aoi[i].title + '</a>\
                    ...
                    if (counts != 0) {
                        ...
                        for (var i = 0; i < counts; i++) {
                            ...
                            style="margin-bottom:0px;"><th><a style="font-size:15px;" href="/soi_detail/' + data.all_soi[i].soi_id + '">' + data.all_soi[i].soi_title + '</a>\
                    ...
                    if (counts != 0) {
                        ...
                        for (var i = 0; i < counts; i++) {
                            ...
                            style="margin-bottom:0px;"><th><a style="font-size:15px;" href="/manage_group/' + data.all_group[i].group_id + '">' + data.all_group[i].group_name + '</a>\
                            <button class="btn btn-success" style="float:right" onclick="Verify(\'' + content + '\',' + data.all_group[i].group_id + ',1)">驗證通過</button>\
                            <button class="btn btn-danger" style="float:right" data-toggle="modal" data-target="#exampleModalCenter" onclick="auto_fill_in(\'' + content + '\',' + data.all_group[i].group_id + ',-1)">驗證不通過</button></th></table>'
                            );
                        }
                    } else {
                        $('.content_detail').append('<h5>共:' + counts + '筆群組</h5>');
...
function Verify(content, id, verification) {
    var urls = '../edit_verification';
    var cfm = confirm("確定驗證?");
    var feedback_mes = "null";
...
function ModalVerify() {
    var cfm = confirm("確定驗證不通過?");

    var urls = '../coi_verification';
    var urls_two = '../edit_verification';    
    ...
    if (cfm == true) {
        ...
        $.ajax({
            ...
            error: function(data) {
                console.log(data);
                console.log("Error with verification2");
            }
        });
        $('#loading').hide();
    }
...
$('#city').change(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            area_select.append('<option value="all" class="all">全部</option>')
